...
static char *battery(const char *);
...
static char *wifi_stat(const char *);
...
static char *
wifi_stat(const char *wificard)
{
	/* wifi strength in percent */
	int strength;
	char buf[255];
	char *datastart;
	char status[5];
	...
	if (strcmp((char *)wreq.u.essid.pointer, "") == 0)
		...
		return smprintf("%s(%d%%)", (char *)wreq.u.essid.pointer, strength);
}
